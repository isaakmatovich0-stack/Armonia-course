...
export async function loadAuthorDisplayMap(codes) {
  ...
  const { data, error } = await supabase
    ...
    .select('code, name, photo_url, instrument')
    .in('code', unique);
  ...
  const map = {};
  data.forEach(row => {
    map[row.code] = { name: row.name || 'Armonía Student', photoUrl: row.photo_url || null, instrument: row.instrument || null };
  });
  // Fallback for any code without a completed profile yet.
  unique.forEach(code => {
    if (!map[code]) map[code] = { name: 'Armonía Student', photoUrl: null, instrument: null };
  });
  return map;
}
