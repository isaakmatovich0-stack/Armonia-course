...
export async function sendGeneratedCodeEmail({ to, code, codeType }) {
  ...
  const intro = isClassroom
    ? `You've been given classroom access to Armonía — built for use on a shared screen with your students. Enter the school and teacher name it asks for on first login, and you're set.`
    : `You've been given complimentary access to Armonía. Your access code is below — this is your key into the course site and Armonía Connect.`;
  const note = isClassroom
    ...
    : `Keep this code private — it's tied to this access grant and works exactly like a purchased code, including full access to every lesson, performance track, and Armonía Connect.`;
  const html = emailShell(`
    <h1 style="font-size:24px; color:#F2EBD9; margin:0 0 16px;">${isClassroom ? 'Classroom Access' : 'Welcome to Armonía'}</h1>
  ...
  return resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: isClassroom ? 'Your Armonía classroom access code' : 'Your Armonía access code is here',
    html,
  });
}
